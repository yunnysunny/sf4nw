...
function output(request,response,pathname,params,isTpl) {
	...
	function existCallback(exist) {
		...
		if (exist) {
			if (isTpl && typeof(params) == 'object') {
				if (typeof(doT) == 'object') {
					tplRender(response,pathname,params);
				} else {
...
function loadTpl(request,response,viewName,params) {
	var filename = GLOBAL_VIEW_PATH + '/' + viewName + '.html';
	output(request, response,filename,params,true);
}
